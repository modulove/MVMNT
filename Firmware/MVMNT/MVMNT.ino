...
void loop()
{
  ...
  if (currentTrigState != prevTrigState) {
    // If Trig is HIGH, store the current output value
    if (currentTrigState == HIGH) {
      holdValue = bz_val * level / 255;
    }
    // If Trig is released, reset the holdValue
    else {
      holdValue = 0;
    }
 ...
 if(timer1 +50 < millis()){
 freq = min(511, (1023 - analogRead(5)) / 2) * freq_dev;
 curve = min(255, (analogRead(1) / 4));
